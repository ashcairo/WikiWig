...
define("CAPTCHA_TIMEOUT", 2000);     // (in seconds/2) = 3:00 hours to solve a displayed captcha
...
$changer .= $WK['systemDir'] . '/captcha/captcha.php' ;
// define("CAPTCHA_BASE_URL", "https://$_SERVER[SERVER_NAME]:$_SERVER[SERVER_PORT]/" . substr(realpath(__FILE__), strlen(realpath($_SERVER["DOCUMENT_ROOT"]))));
define("CAPTCHA_BASE_URL", $changer);
...
class captcha {
   ...
   function solved() {
      // error_log("Solve enter");
      $c = new easy_captcha();
      // error_log("Solve exit");
      return $c->solved();
   }
   function check() {
...
#-- init (triggered if *this* script is called directly)
 if (realpath(strtok("$_SERVER[DOCUMENT_ROOT]/$_SERVER[REQUEST_URI]","?#"))==realpath(__FILE__)) {
    ...
    if (isset($_SERVER['QUERY_STRING']) && strpos($_SERVER['QUERY_STRING'], "__ec_i=ec.") === 0 ){
      $c = new easy_captcha();
      $expired = !$c->is_valid();
      if ($expired) {
	$type = "image/png";
...
class easy_captcha {
   ...
   function solved() {
      ...
      // error_log("solve:3 in: " . $in);
      
         #-- check individual modules
         $ok = $this->grant;
      // error_log("solve:4 ok: " . ($ok ? 'T' : 'F'));
         foreach ($this->shortcut as $test) {
            $ok = $ok || $test->solved($in);    // cookie & nourls
      // error_log("solve:4a ok: " . ($ok ? 'T' : 'F'));
         }
      // error_log("solve:5 ok: " . ($ok ? 'T' : 'F'));
...
//             || isset($this->text) && $this->text->solved($in);
      ...
      // error_log("solve:6 ok: " . ($ok ? 'T' : 'F'));
         $ok = $ok  // either letters or math formula submitted
             || isset($this->text) && $this->text->solved($in);
               
      // error_log("solve:7 ok: " . ($ok ? 'T' : 'F'));
   ...
   function form($add_text="") {
      ...
      $HTML =
         ...
         '&nbsp;' .
         $add_text .
         '<input title="please enter the letters you recognize in the CAPTCHA image to the left" type="text" ' .
                 $onKeyDown.' id="'.
                 $p_input.'" name="'.
                 $p_input.'" value="'.
                 // wtf?
                 // htmlentities($_REQUEST[$p_input]) .
...
class easy_captcha_fuzzy extends easy_captcha {
   ...
   function solved($in) {
      ...
      if ($in) {
         ...
         $res = ($pw == $in) or ($diff <= $maxdiff);  // either matches, or allows around 2 divergent letters
      }
      // error_log("easy_captcha_fuzzy res: " . ($res ? 'T' : 'F'));
      return $res;
   }
...
class easy_captcha_text_math_formula extends easy_captcha {
   ...
   function solved($result) {
      $res = (int)$this->solution == (int)$result;
      // error_log("easy_captcha_text_math_formula res: " . ($res ? 'T' : 'F'));
      return $res;
   }
...
class easy_captcha_persistent_grant extends easy_captcha {
   ...
   function solved($ignore=0) {
      ...
      if (CAPTCHA_PERSISTENT && isset($_COOKIE[$this->cookie()])) {
         $res = in_array($_COOKIE[$this->cookie()], array($this->validity_token(), $this->validity_token(-1)));
      }
      // error_log("easy_captcha_persistent_grant res: " . ($res ? 'T' : 'F'));
      return $res;
   }
...
class easy_captcha_spamfree_no_new_urls {
   ...
   function solved($ignore=0) {
      $res = !preg_match("#(https?://\w+[^/,.]+)#ims", serialize($_GET+$_POST), $uu);
      // error_log("easy_captcha_spamfree_no_new_urls res: " . ($res ? 'T' : 'F'));
      return $res;
   }
}
